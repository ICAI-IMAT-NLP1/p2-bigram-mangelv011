...
def bigrams_count_to_probabilities(
    ...
    # TODO
    bigram_counts_smoothed = bigram_counts + smooth_factor
    row_sums = bigram_counts_smoothed.sum(dim=1, keepdim=True) # obtenemos la suma por cada fila (esto es un tensor columna donde cada elemento es la suma de la fila)
    return bigram_counts_smoothed / row_sums # dividimos cada valor por la suma de su fila
...
def calculate_neg_mean_log_likelihood(
    ...
    # TODO
    total_log_likelihood: torch.tensor = torch.zeros(size=(1,))
    ...
    # TODO
    mean_log_likelihood: float = -1*torch.mean(total_log_likelihood).item()
...
def sample_next_character(
    ...
    # TODO
    current_probs: torch.Tensor[float] = probability_distribution[current_char_index] # fila

    # Sample an index from the distribution using the torch.multinomial function
    # TODO
    # Squeeze to ensure current_probs is 1D if there's an unnecessary extra dimension
    current_probs = current_probs.squeeze()  # Removes any singleton dimensions
    
    # Ensure that current_probs is a 1D tensor
    if current_probs.dim() != 1:
        raise ValueError(f"Expected 1D tensor for probability distribution, got {current_probs.dim()}D")
...
def generate_name(
    ...
    if generated_name.endswith(end_token):
...
def calculate_log_likelihood(
    ...
    # TODO
    log_likelihood: torch.tensor = torch.zeros(size=(1,))
